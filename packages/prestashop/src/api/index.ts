...
export interface PrestashopConfig extends CommerceAPIConfig {
  ...
  storeApiFetch<T>(
    ...
    options?: {
      method?: string
      body?: any
      redirect?: any // TO DO check for categories
      headers?: HeadersInit
    }
  ): Promise<T>
}
...
const STORE_API_URL = process.env.PRESTAHOP_STORE_API_URL // REST API
...
if (!STORE_API_URL) {
  throw new Error(
    `The environment variable PRESTAHOP_STORE_API_URL is missing and it's required to access your store`
  )
}
...
const config: PrestashopConfig = {
  commerceUrl: '', // TO DO: check if relevant
  apiToken: '', // TO DO: check if relevant
  customerCookie: 'SHOP_TOKEN',
  cartCookie: process.env.BIGCOMMERCE_CART_COOKIE ?? 'bc_cartId',
  cartCookieMaxAge: ONE_DAY * 30,
  applyLocale: true,
...
export type PrestashopAPI<P extends Provider = Provider> = CommerceAPI<P>
